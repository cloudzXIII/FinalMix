...
SMODS.Joker {
  ...
  generate_ui = function(self, info_queue, card, desc_nodes, specific_vars, full_UI_table)
    if desc_nodes ~= full_UI_table.main and G.jokers then
      ...
      local munny_area = CardArea(
        G.ROOM.T.x + 0.2 * G.ROOM.T.w / 2,
        G.ROOM.T.h,
        1.05 * G.CARD_W * card_scale,
        0.95 * G.CARD_H * card_scale,
        { card_limit = 1, type = "title", highlight_limit = 0, collection = true }
      )
      local mun = SMODS.add_card({
        key = "j_kh_munny",
        area = munny_area
      })
      mun.T.scale = card_scale
      desc_nodes[#desc_nodes + 1] = { {
        n = G.UIT.R,
        config = { align = "cm", padding = 0.07, no_fill = true },
        nodes = {
          { n = G.UIT.O, config = { object = munny_area } }
        }
      } }
    end
